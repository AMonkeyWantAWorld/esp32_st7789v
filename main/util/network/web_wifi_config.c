    ...
    int32_t event_id, void *event_data)
{
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        _ui_screen_change(&ui_Weather, LV_SCR_LOAD_ANIM_FADE_ON, 200, 14000, &ui_Weather_screen_init);
        s_retry_num = 0;
...
wifi_mode_t app_wifi_main(bool *station_connected)
{
    ...
    *station_connected = false;
    ...
    if (mode & WIFI_MODE_STA) {
        ...
        lv_display_text(set_display_font(ui_SysState, 0, 75, LV_LABEL_LONG_WRAP, LV_ALIGN_CENTER, "正在连接网络。。。"));
    }
